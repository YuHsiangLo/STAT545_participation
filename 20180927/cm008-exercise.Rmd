...
gapminder %>%
  group_by(country) %>%
  mutate(growth = pop - first(pop))  # This will honor the group_by(country)

gapminder %>%
  group_by(country) %>%
  mutate(change = pop - lag(pop))  # This will honor the group_by(country)
```
...
gapminder %>%
  ...
  group_by(country) %>%
  mutate(change = pop - pop[year == 1962])
```
